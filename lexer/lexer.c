...
t_token *next_token(t_lexer *l)
{
	...
	if (l->ch == '|')
	{
		...
		{
			token = new_token(OR_IF, l, 2);
			read_char(l);
		}
		else
			token = new_token(PIPE, l, 1);
	}
	else if (l->ch == '>')
	{
		if (l->input[l->read_position] == '>')
		{
			token = new_token(REDIRECT_APPEND, l, 2);
			read_char(l);
		}
		else
			token = new_token(REDIRECT_OUT, l, 1);
	}
	else if (l->ch == '<')
	{
		if (l->input[l->read_position] == '<')
		{
			token = new_token(HEREDOC, l, 2);
			read_char(l);
		}
		else
			token = new_token(REDIRECT_IN, l, 1);
	}
	else if (l->ch == '&')
	{
		if (l->input[l->read_position] == '&')
		{
			token = new_token(AND_IF, l, 2);
			read_char(l);
		}
		else
			token = new_token(ILLEGAL, l, 1);
	}
	else if (l->ch == '$')
	{
	...
	{
		token = new_token(LPAREN, l, 1);
		l->is_subshell = true;
	}
	else if (l->ch == ')')
	{
		token = new_token(RPAREN, l, 1);
		l->is_subshell = false;
	}
